    ...
    implements IUpdater<WalletUpdaterRequest, WalletUpdaterResponse>
{
    ...
    // lifecycle methods
    async start() {
        // shall we persist pending tasks to the storage?
    }
        ...
        return results
            .map((result) => {
                if (result.status === "fulfilled") {
                    ...
                    // TODO: how to deliver errors in this case? a broadcast?
                    return null;
                }
            })
    ...
    private async handleSignTransaction(message: RequestSignTransaction) {
        ...
        const signature = await this.handler.handleSignTransaction(
            tx,
            inputIndexes
        );
        if (!signature) {
