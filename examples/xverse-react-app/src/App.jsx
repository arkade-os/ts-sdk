...
function App() {
  ...
  const connectWallet = async () => {
    try {
      setLoading(true);
      clearStatus();
      addStatus('Connecting to Xverse wallet...', 'info');

      const info = await wallet.connect();
      setWalletInfo(info);
      
      addStatus('Wallet connected successfully!', 'success');
      addStatus(`ARK address: ${info.arkAddress}`, 'success');
  ...
  const loadWalletData = async () => {
    try {
      ...
      const [walletBalance, walletVtxos, walletBoardingUtxos] = await Promise.all([
        wallet.getBalance(),
        wallet.getVtxos(),
      ...
      addStatus(`Balance loaded: ${walletBalance.total} sats total`, 'success');
      addStatus(`Available: ${walletBalance.available} sats, Boarding: ${walletBalance.boarding.total} sats`, 'info');
      addStatus(`Found ${walletVtxos.length} VTXOs and ${walletBoardingUtxos.length} boarding UTXOs`, 'info');
  ...
  const sendBitcoin = async () => {
    ...
    try {
      setLoading(true);
      clearStatus();
      addStatus('Sending Bitcoin via ARK...', 'info');

      const amountSats = parseInt(amount);

      addStatus('Submitting transaction to ARK server...', 'info');
      ...
      addStatus(`ARK Transaction ID: ${arkTxid}`, 'success');
  ...
  const settleToOnchain = async () => {
    ...
    try {
      ...
      addStatus(`Processed ${result.events.length} settlement events`, 'info');
  ...
  const debugWallet = () => {
    ...
    addStatus(`ARK Server: ${debugInfo.arkServerUrl}`, 'info');
    addStatus(`Esplora: ${debugInfo.esploraUrl}`, 'info');
    addStatus(`Found ${debugInfo.providers.length} wallet providers:`, 'info');
    
    debugInfo.providers.forEach((provider, i) => {
      addStatus(`  ${i + 1}. ${provider.name} (${provider.id})`, 'info');
    });
    
    const xverseFound = debugInfo.providers.some(p => 
      p.name === 'Xverse Wallet' || p.id === 'BitcoinProvider'
    );
    
    if (xverseFound) {
      addStatus('✅ Xverse wallet detected!', 'success');
    } else {
      addStatus('❌ Xverse wallet not found', 'error');
      addStatus('💡 Install Xverse extension and refresh', 'info');
    }
  };

  const isFormValid = walletInfo && recipient.trim() && amount && parseInt(amount) > 0;

  return (
    <div className="container">
      <div className="header">
        <h1>ARK Wallet Demo</h1>
        <p>Connect your Xverse wallet and create cooperative transactions</p>
      </div>
      ...
      <div className="content">
        ...
        <div className="section">
          ...
          <button 
            ...
            disabled={loading}
          >
            {loading ? 'Connecting...' : walletInfo ? 'Connected ✓' : 'Connect Xverse Wallet'}
          </button>
          ...
          <div className="section">
            ...
            <div className="info-grid">
              <div className="info-item">
                ...
                <span>{walletInfo.ordinalAddress}</span>
              </div>
              <div className="info-item">
        ...
        {walletInfo && (
          <div className="section">
            <h2>Send Bitcoin (ARK)</h2>
            <div className="input-group">
              <label>Recipient ARK Address</label>
              <input
                type="text"
                value={recipient}
                onChange={(e) => setRecipient(e.target.value)}
                placeholder="Enter ARK address (tark...)..."
                disabled={loading}
              />
            </div>
            ...
            >
              {loading ? 'Sending...' : 'Send Bitcoin via ARK'}
            </button>
          </div>
        )}
        ...
        {walletInfo && (
          <div className="section">
            ...
            <p style={{ fontSize: '0.9rem', color: '#666', marginBottom: '15px' }}>
              Settle all your funds (boarding UTXOs + VTXOs) to your offchain ARK address.
            </p>
            <button 
              className="button secondary"
        ...
        {transactionResult && (
          <div className="section">
            ...
            <div className="info-item">
              ...
              <span style={{ fontFamily: 'monospace', fontSize: '0.9rem' }}>{transactionResult}</span>
            </div>
            <p style={{ fontSize: '0.9rem', color: '#666', marginTop: '10px' }}>
              Your transaction has been submitted to the ARK server and will be processed in the next batch.
            </p>
          </div>
        )}
          ...
          <div className="status-log">
            {statusMessages.length === 0 ? (
              <div className="status-message info">
                ...
                ARK Wallet Demo - Real Bitcoin transactions via ARK
              </div>
            ) : (
