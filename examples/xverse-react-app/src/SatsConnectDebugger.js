...
export class SatsConnectDebugger {
    ...
    static async testSignPsbtMethods(satsConnectRequest, testPsbt, testAddress) {
    ...
    static logTransaction(tx) {
        ...
        for (let i = 0; i < tx.inputsLength; i++) {
            try {
                ...
                console.log(`Input ${i}:`, {
                    hash: input.hash ? Array.from(input.hash).map(b => b.toString(16).padStart(2, '0')).join('') : 'undefined',
                    index: input.index,
        ...
        for (let i = 0; i < tx.outputsLength; i++) {
            try {
                ...
                console.log(`Output ${i}:`, {
                    amount: output.amount ? output.amount.toString() : 'undefined',
                    script: output.script ? Array.from(output.script).length : 'undefined'
                });
            } catch (e) {
