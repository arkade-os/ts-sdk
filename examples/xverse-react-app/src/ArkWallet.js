...
export class ArkWallet {
  ...
  async connect() {
    try {
      ...
      const xverseProvider = providers.find(p => 
      ...
      const ordinalAccount = addresses.find(addr => 
        addr.purpose === 'ordinals' || addr.purpose === 'payment'
      ) || addresses[0];
      ...
      this.userPubKey = fullPubkey.length === 33 ? fullPubkey.slice(1) : fullPubkey;
  ...
  getDebugInfo() {
    return {
      ...
      arkServerUrl: ARK_SERVER_URL,
      esploraUrl: ESPLORA_URL,
      providers: getProviders().map(p => ({ name: p.name, id: p.id, methods: p.methods }))
    };
  }
}
