...
export class SatsConnectIdentity {
    ...
    async sign(tx, inputIndexes = null) {
        ...
        try {
            const connectionTest = await SatsConnectDebugger.testConnection(this.satsConnectRequest);
            if (!connectionTest.success) {
                console.log('Initial connection test failed, attempting reconnection...');
                const reconnected = await this.reconnect();
                if (!reconnected) {
                    throw new Error(`Wallet not connected: ${connectionTest.error}`);
                }
            }
        ...
        try {
            ...
            if (inputIndexes) {
                ...
                const allIndexes = Array.from({ length: tx.inputsLength }, (_, i) => i);
                signInputs = {
            ...
            if (response.status === "success") {
                ...
                if (signedPsbtBase64) {
                    ...
                    console.error('No PSBT found in successful response:', response);
                    throw new Error('No signed PSBT returned from wallet');
                }
            } else {
                const errorMsg = response.error?.message || "Unknown error";
                const errorCode = response.error?.code || "UNKNOWN";
                console.error('Wallet signing failed:', { errorCode, errorMsg, fullResponse: response });
                ...
                if (errorCode === 'UNKNOWN' || !response.status) {
                    ...
                    const testResults = await SatsConnectDebugger.testSignPsbtMethods(
                ...
                if (errorCode === 'UNAUTHORIZED') {
                    throw new Error('Wallet not connected or unauthorized. Please reconnect your wallet.');
            ...
            if (error.message.includes('network') || error.message.includes('fetch')) {
                throw new Error('Network error: Could not connect to wallet. Please check your wallet connection.');
            }
    ...
    xOnlyPublicKey() {
        ...
        throw new Error(`Invalid public key length: ${this.publicKey.length}. Expected 32 or 33 bytes.`);
    }
    ...
    async reconnect() {
        try {
            ...
            if (response.status === 'success') {
                ...
                const matchingAddress = addresses.find(addr => addr.address === this.address);
