...
export class ArkadePayrollService {
    ...
    try {
      ...
      if (stored) {
        ...
        for (const batch of parsed) {
          ...
          if (batch.approvedAt) batch.approvedAt = new Date(batch.approvedAt);
          if (batch.executedAt) batch.executedAt = new Date(batch.executedAt);
   ...
   */
  createPayroll(params: CreatePayrollParams, createdBy: string): PayrollBatch {
   ...
   */
  updatePayroll(id: string, recipients: Omit<PayrollRecipient, "id">[]): PayrollBatch {
    const batch = this.batches.get(id);
    if (!batch) {
      throw new Error(`Payroll ${id} not found`);
    }
    if (batch.status !== "pending") {
      throw new Error(`Cannot update payroll with status ${batch.status}`);
    }

    batch.recipients = recipients.map((r) => ({ ...r, id: generateId() }));
    batch.totalAmount = batch.recipients.reduce((sum, r) => sum + r.amount, 0);
  ...
  deletePayroll(id: string): void {
    ...
    if (batch.status !== "pending" && batch.status !== "draft") {
      throw new Error(`Cannot delete payroll with status ${batch.status}`);
    }
  ...
  async executePayroll(params: ExecutePayrollParams): Promise<string> {
    ...
    if (batch.status !== "pending" && batch.status !== "approved") {
      throw new Error(`Cannot execute payroll with status ${batch.status}`);
    }
    ...
    try {
      ...
      batch.status = "failed";
      batch.errorMessage = error instanceof Error ? error.message : String(error);
  ...
  approvePayroll(id: string, approvedBy: string): PayrollBatch {
    ...
    if (batch.status !== "pending") {
      throw new Error(`Cannot approve payroll with status ${batch.status}`);
    }
