...
export function parseCsv(content: string): CsvParseResult {
  ...
  const startIndex = firstLine.includes("address") || firstLine.includes("amount") ? 1 : 0;
  ...
  for (let i = startIndex; i < lines.length; i++) {
    ...
    const amountSats = amountStr.includes(".") && parseFloat(amountStr) < 1
...
export function generateCsv(recipients: PayrollRecipient[]): string {
  ...
  const rows = recipients.map((r) => `${r.address},${r.amount},${r.name || ""}`);
