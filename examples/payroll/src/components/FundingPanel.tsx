...
import { lendaswapService, type SwapQuote, type SwapOrder } from "../services/lendaswap";
...
export function FundingPanel({ arkadeAddress, requiredAmount, onFunded }: FundingPanelProps) {
  ...
  const [step, setStep] = useState<"quote" | "deposit" | "claim" | "complete">("quote");
  ...
  const handleGetQuote = async () => {
    ...
    try {
      ...
      const amountSmallest = BigInt(Math.floor(parseFloat(usdtAmount) * 1_000_000));
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to claim on Arkade");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="funding-panel">
      ...
      {step === "quote" && (
        <div className="step-content">
          <div className="funding-hint">
            ...
            <span className="usd-equiv">(~${requiredUsd.toFixed(2)} USD)</span>
          </div>
            ...
            <div className="quote-result">
              ...
              <div className="quote-grid">
                <div className="quote-item">
                  <span>You send:</span>
                  <strong>${(Number(quote.sourceAmount) / 1_000_000).toFixed(2)} USDT</strong>
                </div>
                <div className="quote-item">
                  <span>You receive:</span>
                  <strong>{formatAmount(Number(quote.targetAmount))}</strong>
                </div>
                <div className="quote-item">
                  <span>Protocol fee:</span>
                  <span>{formatAmount(Number(quote.protocolFee))}</span>
                </div>
                <div className="quote-item">
                  <span>Expires:</span>
                  <span>{quote.expiresAt.toLocaleTimeString()}</span>
                </div>
              </div>
      ...
      {step === "deposit" && order && (
        <div className="step-content">
          ...
          <div className="deposit-instructions">
            ...
            <div className="instruction-item">
              <span>Amount:</span>
              <strong>${(Number(order.sourceAmount) / 1_000_000).toFixed(2)} USDT</strong>
            </div>
            <div className="instruction-item">
              <span>Hash Lock:</span>
            ...
            <div className="instruction-item">
              <span>Timelock:</span>
              <span>{new Date(order.timelock * 1000).toLocaleString()}</span>
            </div>
          </div>

          <div className="form-group">
            <label htmlFor="tx-hash">Ethereum Transaction Hash</label>
            <input
              id="tx-hash"
              type="text"
              placeholder="0x..."
      ...
      {step === "claim" && (
        <div className="step-content">
          ...
          <p>Your USDT deposit has been confirmed. Click below to claim your BTC on Arkade.</p>
      ...
      {step === "complete" && (
        <div className="step-content success">
          ...
          <button className="btn-secondary" onClick={() => setStep("quote")}>
            Make Another Swap
          </button>
        </div>
      )}
