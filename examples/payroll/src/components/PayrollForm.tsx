...
interface RecipientRowProps {
  ...
  index: number;
  onUpdate: (index: number, field: keyof PayrollRecipient, value: string | number) => void;
  onRemove: (index: number) => void;
}

function RecipientRow({ recipient, index, onUpdate, onRemove }: RecipientRowProps) {
  return (
    <div className="recipient-row">
      ...
      <input
        ...
        onChange={(e) => onUpdate(index, "amount", parseInt(e.target.value) || 0)}
        className="input-amount"
        min="1"
        required
      />
      <button type="button" onClick={() => onRemove(index)} className="btn-remove" title="Remove">
        -
      </button>
    </div>
  );
...
export function PayrollForm({ onCreated }: PayrollFormProps) {
  ...
  const [recipients, setRecipients] = useState<(Omit<PayrollRecipient, "id"> & { id?: string })[]>([
    { address: "", amount: 0 },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validRecipients = recipients.filter((r) => r.address && r.amount > 0);
      if (validRecipients.length === 0) {
        validationErrors.push("At least one valid recipient is required");
      }

      for (let i = 0; i < recipients.length; i++) {
        const r = recipients[i];
        if (r.address && r.address.length < 10) {
          validationErrors.push(`Recipient ${i + 1}: Invalid address`);
        }
      ...
      setErrors([error instanceof Error ? error.message : "Failed to create payroll"]);
    } finally {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-footer">
          <div className="total">
            ...
            <span className="recipient-count">({recipients.filter((r) => r.address).length} recipients)</span>
          </div>
