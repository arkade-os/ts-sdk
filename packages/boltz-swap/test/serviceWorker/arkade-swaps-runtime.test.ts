...
describe("SwArkadeSwapsRuntime events", () => {
    ...
    it("subscribeToSwapUpdates filters by swapId and unsubscribes", async () => {
        ...
        const spy = vi.fn();
        const unsubscribe = await mgr.subscribeToSwapUpdates("target-swap", spy);
...
describe("SwArkadeSwapsRuntime enrich methods", () => {
    ...
    it("enrichReverseSwapPreimage throws on hash mismatch", async () => {
        ...
        expect(() => runtime.enrichReverseSwapPreimage(swap, "11".repeat(32))).toThrow(
    ...
    it("enrichSubmarineSwapInvoice throws for invalid invoice", async () => {
        ...
        expect(() => runtime.enrichSubmarineSwapInvoice(swap, "not-a-lightning-invoice")).toThrow(
...
describe("sendMessage reinitialize on SW restart", () => {
    ...
    it("throws after exhausting retries", async () => {
        ...
        await expect(runtime.refreshSwapsStatus()).rejects.toThrow(MESSAGE_BUS_NOT_INITIALIZED);
    ...
    it("deduplicates concurrent reinitializations", async () => {
        ...
        const [fees, limits] = await Promise.all([runtime.getFees(), runtime.getLimits()]);
    ...
    it("does not retry for errors other than 'not initialized'", async () => {
        ...
        const runtime = createRuntimeWithConfig(serviceWorker as any);
        // refreshSwapsStatus doesn't wrap errors, so we see the raw message
        await expect(runtime.refreshSwapsStatus()).rejects.toThrow("something else went wrong");
...
describe("in-flight request deduplication", () => {
    ...
    it("deduplicates concurrent identical reads", async () => {
        ...
        const runtime = createRuntimeWithConfig(serviceWorker as any);
        const [f1, f2] = await Promise.all([runtime.getFees(), runtime.getFees()]);
    ...
    it("does not dedup state-mutating requests", async () => {
        ...
        await Promise.all([runtime.createReverseSwap(args), runtime.createReverseSwap(args)]);
    ...
    it("deduplicates requests with identical payloads", async () => {
        ...
        const runtime = createRuntimeWithConfig(serviceWorker as any);
        await Promise.all([runtime.getSwapStatus("swap-123"), runtime.getSwapStatus("swap-123")]);
    ...
    it("does NOT dedup different payloads", async () => {
        ...
        const runtime = createRuntimeWithConfig(serviceWorker as any);
        await Promise.all([runtime.getSwapStatus("swap-aaa"), runtime.getSwapStatus("swap-bbb")]);
    ...
    it("shares error across deduped callers", async () => {
        ...
        const runtime = createRuntimeWithConfig(serviceWorker as any);
        const results = await Promise.allSettled([runtime.getFees(), runtime.getFees()]);
...
describe("message timeouts", () => {
    ...
    it("does not apply the page-side deadline to long-running requests", async () => {
        ...
        const { navigatorServiceWorker, serviceWorker, emit } = createServiceWorkerHarness();
        ...
        const request = serviceWorker.postMessage.mock.calls
            ...
            .find((message: any) => message.type === "WAIT_FOR_SWAP_SETTLEMENT");

        expect(request).toEqual(expect.objectContaining({ type: "WAIT_FOR_SWAP_SETTLEMENT" }));
