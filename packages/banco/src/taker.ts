...
export class Taker {
     ...
     */
    async fulfillByTxid(txid: string, options?: FulfillOptions): Promise<{ txid: string }> {
        ...
        if (!bancoPacket) {
            throw new Error(`No Banco offer packet found in transaction ${txid}`);
        }
     ...
     * @returns The ark transaction id of the fulfillment.
     */
    async fulfill(offerHex: string, options?: FulfillOptions): Promise<{ txid: string }> {
    ...
    private async fulfillOffer(
        ...
        const fulfillMultisig = MultisigTapscript.encode({
            pubkeys: [
                serverPubKey,
                arkade.computeArkadeScriptPublicKey(offer.emulatorPubkey, covenantScriptBytes),
            ],
        });
        const swapTapLeafScript = swapVtxoScript.findLeaf(hex.encode(fulfillMultisig.script));
        ...
        const fillAmount =
            isPartial && options?.fillAmount !== undefined ? options.fillAmount : offer.wantAmount;
        ...
        if (wantAssetStr) {
            ...
            if (selectedBtc < Number(DUST)) {
                const remaining = allTakerVtxos.filter(
                    (c) => !selected.find((sc) => sc.txid === c.txid && sc.vout === c.vout),
                );
            ...
            const { inputs } = selectVirtualCoins(allTakerVtxos, Number(fillAmount));
            selectedTakerVtxos = inputs;
        }

        const totalTakerBtc = BigInt(selectedTakerVtxos.reduce((s, v) => s + v.value, 0));
        ...
        if (isPartial) {
            ...
            if (isAssetOffer) {
                ...
                // ────────────────────────────────
                const offerEntry = swapVtxo.assets?.find((a) => a.assetId === offerAssetStr);
                if (!offerEntry) {
                    throw new Error("Swap VTXO does not carry the offered asset");
                }
                ...
                if (totalTakerBtc < makerBtc) {
                    throw new Error(`Insufficient BTC: have ${totalTakerBtc}, need ${makerBtc}`);
                }
                ...
                if (isFullFill) {
                    offerOutputs.push(asset.AssetOutput.create(takerOutputIdx, offerEntry.amount));
                } else {
                    offerOutputs.push(
                        asset.AssetOutput.create(0, Number(swapOfferAmount - consumed)),
                        asset.AssetOutput.create(takerOutputIdx, Number(consumed)),
                    );
                }
                assetGroups.push(
                    ...
                    for (const a of swapVtxo.assets) {
                        ...
                        assetGroups.push(
                            asset.AssetGroup.create(
                                ...
                                [asset.AssetOutput.create(takerOutputIdx, a.amount)],
                                [],
                            ),
                        );
                    }
                ...
                if (offer.wantAsset) {
                    ...
                    for (let i = 0; i < selectedTakerVtxos.length; i++) {
                        ...
                        const entry = vtxo.assets?.find((a) => a.assetId === wantAssetStr);
                        if (entry) {
                            wantInputs.push(asset.AssetInput.create(i + 1, entry.amount));
                        }
                    }
                    const wantOutputs: asset.AssetOutput[] = [
                        asset.AssetOutput.create(makerOutputIdx, Number(fillAmount)),
                    ];
                    if (takerAssetChange > 0n) {
                        wantOutputs.push(
                            asset.AssetOutput.create(takerOutputIdx, Number(takerAssetChange)),
                        );
                    }
                    assetGroups.push(
                        asset.AssetGroup.create(offer.wantAsset, null, wantInputs, wantOutputs, []),
                    );
                }
            ...
            if (offer.wantAsset) {
                ...
                for (let i = 0; i < selectedTakerVtxos.length; i++) {
                    ...
                    const entry = vtxo.assets?.find((a) => a.assetId === wantAssetStr);
                    if (entry) {
                        assetInputs.push(asset.AssetInput.create(i + 1, entry.amount));
                    }
                }
                ...
                if (takerAssetChange > 0n) {
                    assetOutputs.push(asset.AssetOutput.create(1, Number(takerAssetChange)));
                }
                assetGroups.unshift(
                    asset.AssetGroup.create(offer.wantAsset, null, assetInputs, assetOutputs, []),
                );
            }
            ...
            if (btcChange < 0n) {
                throw new Error(`Insufficient BTC: have ${totalTakerBtc}, need ${makerBtcAmount}`);
            }
        ...
        if (isPartial) {
            const { txs: fundingTxs } = await this.indexer.getVirtualTxs([swapVtxo.txid]);
            if (fundingTxs.length === 0) {
                throw new Error(`Funding tx ${swapVtxo.txid} not found at indexer`);
            }
            const fundingTx = Transaction.fromPSBT(base64.decode(fundingTxs[0]));
            setArkPsbtField(arkTx, 0, PrevArkTxField, fundingTx.unsignedTx);
        }

        const takerInputIndexes = takerInputs.map((_, i) => i + 1);
        const signedArkTx = await this.wallet.identity.sign(arkTx, takerInputIndexes);
        ...
        const finalArkTx = Transaction.fromPSBT(base64.decode(emuResult.signedArkTx));
    ...
    private collectCollateral(
        ...
        const collateral = new Map<string, { inputs: asset.AssetInput[]; total: number }>();
