...
describe("Indexer provider", () => {
    ...
    it("should inspect a VTXO", { timeout: 60000 }, async () => {
        ...
        const spendableVtxosResponse = await indexerProvider.getVtxos({
            scripts: [hex.encode(ArkAddress.decode(aliceOffchainAddress!).pkScript)],
            spendableOnly: true,
        });
    ...
    it("should inspect a commitment tx", { timeout: 120_000 }, async () => {
        ...
        const forfeitsResponse = await indexerProvider.getCommitmentTxForfeitTxs(txid);
    ...
    it("should subscribe to scripts", { timeout: 60000 }, async () => {
        ...
        fixtures.forEach(({ address, amount, delayMilliseconds }) => {
            setTimeout(() => faucetOffchain(address, amount), delayMilliseconds);
        });

        const subscriptionId = await indexerProvider.subscribeForScripts([hex.encode(aliceScript)]);
        ...
        for await (const update of subscription) {
            ...
            if (now - start < 3 * delayMilliseconds) {
                ...
                await indexerProvider.subscribeForScripts([hex.encode(bobScript)], subscriptionId);
            } else {
        ...
        await indexerProvider.unsubscribeForScripts(subscriptionId, [hex.encode(aliceScript)]);
    ...
    it("should get vtxo chain", { timeout: 60000 }, async () => {
        ...
        const commitmentChainTx = chainResponse.chain.find((tx) => tx.txid === commitmentTxid);
        ...
        expect(chainResponseAfterOffchain.chain.length).toBeGreaterThan(chainResponse.chain.length);

        // verify that the new chain is composed by the previous chain + the checkpoint + the ark tx
        expect(chainResponseAfterOffchain.chain.length).toBe(chainResponse.chain.length + 2);

        // every tx of the initial chain should be in the new chain
        for (const tx of chainResponse.chain) {
            expect(chainResponseAfterOffchain.chain.some((t) => t.txid === tx.txid)).toBe(true);
        }
    ...
    it("should get vtxo tree txs", { timeout: 60000 }, async () => {
        ...
        for (const vtxoTreeTx of treeResponse.vtxoTree) {
            const virtualTxs = await indexerProvider.getVirtualTxs([vtxoTreeTx.txid]);
            expect(virtualTxs.txs).toBeDefined();
    ...
    it("should get connectors from commitment tx", { timeout: 60000 }, async () => {
        const alice = await createTestArkWallet();
        ...
        const connectors = await indexerProvider.getCommitmentTxConnectors(commitmentTxid);
        ...
        for (const connector of connectors.connectors) {
            const virtualTxs = await indexerProvider.getVirtualTxs([connector.txid]);
            expect(virtualTxs.txs).toBeDefined();
            expect(virtualTxs.txs.length).toBe(1);
            const virtualTx = virtualTxs.txs[0];
