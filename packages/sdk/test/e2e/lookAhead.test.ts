...
describe("HD look-ahead band", () => {
    ...
    it("credits an externally issued default-variant address on a delegate wallet", {
        timeout: 120000,
    }, async () => {
        ...
        try {
            ...
            const issued = new DefaultVtxo.Script({
                pubKey: deriveDescriptorLeafPubKey(provider.materializeDescriptorAt(ISSUED_INDEX)),
            ...
            await waitFor(
                async () => (await indexer.getVtxos({ scripts: [issuedScript] })).vtxos.length > 0,
