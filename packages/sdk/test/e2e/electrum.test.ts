...
describe("ElectrumOnchainProvider integration tests", () => {
    ...
    it("exposes the regtest chain tip via the headers subscription", {
        timeout: 10_000,
    }, async () => {
    ...
    it("tracks a freshly-funded address through getCoins / getTransactions / getTxStatus", {
        timeout: 30_000,
    }, async () => {
        ...
        const fundingTx = txs.find((tx) =>
            tx.vout.some(
                (out) => out.scriptpubkey_address === wallet.address && Number(out.value) === sats,
            ),
        );
    ...
    it("broadcasts a single transaction and surfaces the new outpoint", {
        timeout: 30_000,
    }, async () => {
        // Stand up Alice with the electrum provider and Bob (recipient).
        const alice = await OnchainWallet.create(SingleKey.fromRandomBytes(), "regtest", provider);
        const bob = await OnchainWallet.create(SingleKey.fromRandomBytes(), "regtest", provider);
    ...
    it("computes outspends correctly for a tx whose outputs are partially spent", {
        timeout: 30_000,
    }, async () => {
        const alice = await OnchainWallet.create(SingleKey.fromRandomBytes(), "regtest", provider);
        const bob = await OnchainWallet.create(SingleKey.fromRandomBytes(), "regtest", provider);
    ...
    it("exposes WsElectrumChainSource.fetchHistories as a batch round-trip", {
        timeout: 40_000,
    }, async () => {
...
describe("OnchainWallet over ElectrumOnchainProvider", () => {
    ...
    it("performs a complete onchain roundtrip payment via electrum", {
        timeout: 30_000,
    }, async () => {
        const alice = await OnchainWallet.create(SingleKey.fromRandomBytes(), "regtest", provider);
        const bob = await OnchainWallet.create(SingleKey.fromRandomBytes(), "regtest", provider);
