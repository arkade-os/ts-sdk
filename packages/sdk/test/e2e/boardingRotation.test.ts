...
describe("Boarding HD rotation - multi-address discovery & sweep", () => {
    ...
    it("discovers and sweeps boarding UTXOs across current + rotated addresses", {
        timeout: 180_000,
    }, async () => {
...
describe("Boarding HD rotation - rotate-on-board", () => {
    ...
    it("rotates the boarding address after a boarding UTXO is boarded into Arkade", {
        timeout: 120_000,
    }, async () => {
        const mnemonic = generateMnemonic(wordlist);
        const repos = createSharedRepos();
...
describe("Boarding HD rotation - restore()", () => {
    ...
    it("rediscovers boarding funds at a rotated index a fresh same-seed repo cannot see", {
        timeout: 180_000,
    }, async () => {
        const mnemonic = generateMnemonic(wordlist);
