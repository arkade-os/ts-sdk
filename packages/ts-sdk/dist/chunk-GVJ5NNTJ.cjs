...
var musig2 = require('@scure/btc-signer/musig2.js');
var utils_js$1 = require('@noble/curves/utils.js');
var secp256k1 = require('@noble/secp256k1');
var secp256k1_js = require('@noble/curves/secp256k1.js');
var script_js = require('@scure/btc-signer/script.js');
var transaction_js = require('@scure/btc-signer/transaction.js');
...
var celJs = require('@marcbachmann/cel-js');
...
var musig2__namespace = /*#__PURE__*/_interopNamespace(musig2);

function generateNonces(publicKey) {
  const nonces = musig2__namespace.nonceGen(publicKey);
  return { secNonce: nonces.secret, pubNonce: nonces.public };
}
function aggregateNonces(pubNonces) {
  return musig2__namespace.nonceAggregate(pubNonces);
}
function aggregateKeys(publicKeys, sort, options = {}) {
  {
    publicKeys = musig2__namespace.sortKeys(publicKeys);
  }
  const { aggPublicKey: preTweakedKey } = musig2__namespace.keyAggregate(publicKeys);
  ...
  const tweakBytes = secp256k1_js.schnorr.utils.taggedHash(
    "TapTweak",
    preTweakedKey.toBytes(true).subarray(1),
    options.taprootTweak ?? new Uint8Array(0)
  );
  const { aggPublicKey: finalKey } = musig2__namespace.keyAggregate(publicKeys, [tweakBytes], [true]);
  return {
    preTweakedKey: preTweakedKey.toBytes(true),
...
var PartialSig = class _PartialSig {
  ...
  static decode(bytes) {
    ...
    const s = utils_js$1.bytesToNumberBE(bytes);
    if (s >= secp256k1.Point.CURVE().n) {
...
function sign(secNonce, privateKey, combinedNonce, publicKeys, message, options) {
  ...
  if (options?.taprootTweak !== void 0) {
    const { preTweakedKey } = aggregateKeys(
      musig2__namespace.sortKeys(publicKeys) );
    tweakBytes = secp256k1_js.schnorr.utils.taggedHash(
      "TapTweak",
      preTweakedKey.subarray(1),
      options.taprootTweak
    );
  }
  const session = new musig2__namespace.Session(
    combinedNonce,
    musig2__namespace.sortKeys(publicKeys) ,
    message,
...
var TreeSignerSession = class _TreeSignerSession {
  ...
  static random() {
    const secretKey = utils_js.randomPrivateKeyBytes();
  ...
  async getPublicKey() {
    return secp256k1_js.secp256k1.getPublicKey(this.secretKey);
  }
  async getNonces() {
  ...
  async aggregatedNonces(txid, noncesByPubkey) {
    ...
    const cosigners = chunkISZA7V2J_cjs.getArkPsbtFields(tx.root, 0, chunkISZA7V2J_cjs.CosignerPublicKey).map(
  ...
  generateNonces() {
    ...
    const publicKey = secp256k1_js.secp256k1.getPublicKey(this.secretKey);
    for (const g of this.graph.iterator()) {
  ...
  signPartial(g) {
    ...
    const cosigners = chunkISZA7V2J_cjs.getArkPsbtFields(g.root, 0, chunkISZA7V2J_cjs.CosignerPublicKey).map((c) => c.key);
    ...
    const message = g.root.preimageWitnessV1(
      ...
      // always first input
      prevoutScripts,
      transaction_js.SigHash.DEFAULT,
      prevoutAmounts
    );
    return sign(
      myNonce.secNonce,
...
function getPrevOutput(finalKey, graph, sharedOutputAmount, tx) {
  const pkScript = script_js.Script.encode(["OP_1", finalKey.slice(1)]);
  if (tx.id === graph.txid) {
    return {
  ...
  const parentTxid = base.hex.encode(parentInput.txid);
...
var ALL_SIGHASH = Object.values(btcSigner.SigHash).filter((x) => typeof x === "number");
var SingleKey = class _SingleKey {
  key;
  constructor(key) {
    this.key = key || utils_js.randomPrivateKeyBytes();
  }
  ...
  static fromHex(privateKeyHex) {
    return new _SingleKey(base.hex.decode(privateKeyHex));
  }
  /** Create a signing identity with a freshly generated random private key. */
  static fromRandomBytes() {
    return new _SingleKey(utils_js.randomPrivateKeyBytes());
  }
  /**
   ...
   */
  toHex() {
    return base.hex.encode(this.key);
  }
  ...
  compressedPublicKey() {
    return Promise.resolve(utils_js.pubECDSA(this.key, true));
  }
  xOnlyPublicKey() {
    return Promise.resolve(utils_js.pubSchnorr(this.key));
  }
  signerSession() {
...
var ALL_SIGHASH2 = Object.values(btcSigner.SigHash).filter((x) => typeof x === "number");
...
var SeedIdentity = class _SeedIdentity {
  ...
  constructor(seed, opts = {}) {
    ...
    if ("descriptor" in opts && typeof opts.descriptor === "string") {
      ...
      network = chunkJH7WWDEA_cjs.isMainnetDescriptor(descriptor) ? descriptorsScure.networks.bitcoin : descriptorsScure.networks.testnet;
    } else {
      ...
      descriptor = descriptorsScure.scriptExpressions.trBIP32({
        masterNode: descriptorsScure.HDKey.fromMasterSeed(seed, network.bip32),
        network,
        account: 0,
        change: 0,
        index: "*"
    ...
    const masterNode = descriptorsScure.HDKey.fromMasterSeed(seed, network.bip32);
  ...
  async xOnlyPublicKey() {
    return utils_js.pubSchnorr(this.derivedKey);
  }
  async compressedPublicKey() {
    return utils_js.pubECDSA(this.derivedKey, true);
  }
  async sign(tx, inputIndexes) {
   ...
   */
  isOurs(descriptor) {
    return chunkJH7WWDEA_cjs.descriptorIsOurs(descriptor, this.descriptor, utils_js.pubSchnorr(this.derivedKey));
  }
  /**
  ...
  derivePrivateKeyForDescriptor(descriptor) {
    const network = chunkJH7WWDEA_cjs.isMainnetDescriptor(descriptor) ? descriptorsScure.networks.bitcoin : descriptorsScure.networks.testnet;
  ...
  signMessageWithKey(key, message, signatureType) {
    if (signatureType === "ecdsa") return secp256k1.signAsync(message, key, { prehash: false });
    return secp256k1.schnorr.signAsync(message, key);
  }
};
var MnemonicIdentity = class _MnemonicIdentity extends SeedIdentity {
  constructor(phrase, opts) {
    const { passphrase } = opts;
    super(bip39.mnemonicToSeedSync(phrase, passphrase), opts);
  ...
  static fromMnemonic(phrase, opts = {}) {
    if (!bip39.validateMnemonic(phrase, english_js.wordlist)) {
...
var ReadonlyDescriptorIdentity = class _ReadonlyDescriptorIdentity {
   ...
   */
  isOurs(descriptor) {
    return chunkJH7WWDEA_cjs.descriptorIsOurs(descriptor, this.descriptor, this.indexZero.pubkey);
  }
};
...
async function serializeReadonlyIdentity(identity) {
  ...
  return {
    ...
    publicKey: base.hex.encode(await identity.compressedPublicKey())
  };
}
function hydrateIdentity(s) {
  switch (s.type) {
    ...
    case "readonly-single-key":
      return ReadonlySingleKey.fromPublicKey(base.hex.decode(s.publicKey));
    case "seed":
      return SeedIdentity.fromSeed(base.hex.decode(s.seed), {
...
var RestDelegateProvider = class {
  ...
  async delegate(intent, forfeitTxs, options) {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        intent: {
          message: chunkISZA7V2J_cjs.Intent.encodeMessage(intent.message),
          proof: intent.proof
        },
...
var EsploraProvider = class {
  constructor(baseUrl = ESPLORA_URL[chunk4QHMS5XH_cjs.DEFAULT_NETWORK_NAME], opts) {
...
var hexP2Ascript = base.hex.encode(P2A.script);
function findP2AOutput(tx) {
  for (let i = 0; i < tx.outputsLength; i++) {
    const output = tx.getOutput(i);
    if (output.script && base.hex.encode(output.script) === hexP2Ascript) {
...
function buildForfeitTxWithOutput(inputs, output, txLocktime) {
  const tx = new chunkISZA7V2J_cjs.Transaction({
    version: 3,
    lockTime: txLocktime
  });
  for (const input of inputs) {
...
function validateConnectorsTxGraph(settlementTxB64, connectorsGraph) {
  ...
  const settlementTx = transaction_js.Transaction.fromPSBT(base.base64.decode(settlementTxB64));
  ...
  if (base.hex.encode(rootInput.txid) !== expectedRootTxid) throw ErrWrongSettlementTxid;
...
function validateVtxoTxGraph(graph, roundTransaction, sweepTapTreeRoot) {
  ...
  if (!rootInput.txid || base.hex.encode(rootInput.txid) !== commitmentTxid || rootInput.index !== BATCH_OUTPUT_VTXO_INDEX) {
  ...
  for (const g of graph.iterator()) {
    for (const [childIndex, child] of g.children) {
      ...
      const cosigners = chunkISZA7V2J_cjs.getArkPsbtFields(child.root, 0, chunkISZA7V2J_cjs.CosignerPublicKey);
      ...
      if (!finalKey || base.hex.encode(finalKey.slice(1)) !== base.hex.encode(previousScriptKey)) {
...
var EmulatorPacket = class _EmulatorPacket {
  ...
  static fromBytes(data) {
    const reader = new chunkISZA7V2J_cjs.BufferReader(data);
  ...
  serialize() {
    const writer = new chunkISZA7V2J_cjs.BufferWriter();
...
var Extension = class _Extension {
  ...
  static isExtension(script) {
    try {
      const decoded = btcSigner.Script.decode(script);
      if (decoded.length < 2 || decoded[0] !== "RETURN") return false;
      const data = decoded[1];
      if (!(data instanceof Uint8Array)) return false;
      return data.length >= ARKADE_MAGIC.length && utils_js.equalBytes(data.slice(0, ARKADE_MAGIC.length), ARKADE_MAGIC);
    } catch {
      return false;
    }
  }
  ...
  static fromBytes(script) {
    ...
    let decoded;
    try {
      decoded = btcSigner.Script.decode(script);
    } catch {
    ...
    if (payload.length < ARKADE_MAGIC.length || !utils_js.equalBytes(payload.slice(0, ARKADE_MAGIC.length), ARKADE_MAGIC)) {
      throw new Error(
    ...
    const reader = new chunkISZA7V2J_cjs.BufferReader(payload.slice(ARKADE_MAGIC.length));
    const packets = [];
  ...
  getAssetPacket() {
    for (const p of this.packets) {
      if (p instanceof chunkISZA7V2J_cjs.Packet) {
        return p;
      }
    }
    return null;
...
function parsePacket(packetType, data) {
  switch (packetType) {
    case chunkISZA7V2J_cjs.Packet.PACKET_TYPE:
      return chunkISZA7V2J_cjs.Packet.fromBytes(data);
...
function validateBatchRecipients(commitmentTx, vtxoTreeLeaves, recipients, network) {
  ...
  for (const recipient of recipients) {
    let arkAddress;
    try {
      arkAddress = chunk4QHMS5XH_cjs.ArkAddress.decode(recipient.address);
    } catch {
...
function validateOnchainRecipient(commitmentTx, recipient, network, usedOutputs) {
  ...
  for (let i = 0; i < commitmentTx.outputsLength; i++) {
    ...
    if (utils_js.equalBytes(output.script, expectedPkScript)) {
...
function createAssetPacket(assetInputs, receivers, changeReceiver) {
  ...
  for (const [inputIndex, assets] of assetInputs) {
    for (const asset of assets) {
      ...
      inputsByAssetId.set(asset.assetId, [
        ...existing ?? [],
        chunkISZA7V2J_cjs.AssetInput.create(inputIndex, asset.amount)
      ]);
    }
  }
    ...
    if (receiver.assets) {
      for (const asset of receiver.assets) {
        ...
        outputsByAssetId.set(asset.assetId, [
          ...existing ?? [],
          chunkISZA7V2J_cjs.AssetOutput.create(outputIndex, asset.amount)
        ]);
      }
    }
    outputIndex++;
  ...
  for (const assetIdStr of allAssetIds) {
    ...
    const assetId = chunkISZA7V2J_cjs.AssetId.fromString(assetIdStr);
    const group = chunkISZA7V2J_cjs.AssetGroup.create(assetId, null, inputs ?? [], outputs ?? [], []);
    groups.push(group);
  }
  return chunkISZA7V2J_cjs.Packet.create(groups);
}
...
function buildOffchainTx(inputs, outputs, serverUnrollScript) {
  ...
  for (const [index, output] of outputs.entries()) {
    ...
    const isOpReturn = isExtension || btcSigner.Script.decode(output.script)[0] === "RETURN";
    if (isOpReturn) {
      countOpReturn++;
    }
...
function buildVirtualTx(inputs, outputs) {
  let lockTime = 0n;
  for (const input of inputs) {
    const tapscript = chunk4QHMS5XH_cjs.decodeTapscript(chunk4QHMS5XH_cjs.scriptFromTapLeafScript(input.tapLeafScript));
    if (chunk4QHMS5XH_cjs.CLTVMultisigTapscript.is(tapscript)) {
      if (lockTime !== 0n) {
  ...
  for (const [i, input] of inputs.entries()) {
    tx.addInput({
      ...
      witnessUtxo: {
        script: chunk4QHMS5XH_cjs.VtxoScript.decode(input.tapTree).pkScript,
        amount: BigInt(input.value)
      },
      tapLeafScript: [input.tapLeafScript]
    });
    chunkISZA7V2J_cjs.setArkPsbtField(tx, i, chunkISZA7V2J_cjs.VtxoTaprootTree, input.tapTree);
  }
...
function buildCheckpointTx(vtxo, serverUnrollScript) {
  const collaborativeClosure = chunk4QHMS5XH_cjs.decodeTapscript(chunk4QHMS5XH_cjs.scriptFromTapLeafScript(vtxo.tapLeafScript));
  const checkpointVtxoScript = new chunk4QHMS5XH_cjs.VtxoScript([
  ...
  const collaborativeLeafProof = checkpointVtxoScript.findLeaf(
    base.hex.encode(collaborativeClosure.script)
  );
...
function verifyTapscriptSignatures(tx, inputIndex, requiredSigners, excludePubkeys = [], allowedSighashTypes = [btcSigner.SigHash.DEFAULT]) {
  ...
  for (const [tapScriptSigData, signature] of input.tapScriptSig) {
    ...
    const pubKeyHex = base.hex.encode(pubKey);
    ...
    const leafHashHex = base.hex.encode(leafHash);
    let matchingScript;
    let matchingVersion;
    for (const [_, scriptWithVersion] of input.tapLeafScript) {
      ...
      const computedLeafHash = payment_js.tapLeafHash(script, version);
      const computedHex = base.hex.encode(computedLeafHash);
    ...
    if (!matchingScript || matchingVersion === void 0) {
      throw new Error(
        `Input ${inputIndex}: No tapLeafScript found matching leafHash ${base.hex.encode(leafHash)}`
      );
    }
    ...
    const isValid = secp256k1_js.schnorr.verify(sig, message, pubKey);
    if (!isValid) {
      throw new Error(`Invalid signature for input ${inputIndex}, pubkey ${pubKeyHex}`);
    }
  }
  const signedPubkeys = input.tapScriptSig.map(([data]) => base.hex.encode(data.pubKey));
...
function isValidArkAddress(address) {
  try {
    chunk4QHMS5XH_cjs.ArkAddress.decode(address);
    return true;
  } catch (e) {
    return false;
  }
...
var IntentOutputEnv = new celJs.Environment().registerVariable(AmountVariableName, "double").registerVariable(OutputScriptVariableName, "string").registerFunction(nowFunction.signature, nowFunction.implementation);
var IntentOffchainInputEnv = new celJs.Environment().registerVariable(AmountVariableName, "double").registerVariable(ExpiryVariableName, "double").registerVariable(BirthVariableName, "double").registerVariable(WeightVariableName, "double").registerVariable(InputTypeVariableName, "string").registerFunction(nowFunction.signature, nowFunction.implementation);
var IntentOnchainInputEnv = new celJs.Environment().registerVariable(AmountVariableName, "double").registerFunction(nowFunction.signature, nowFunction.implementation);
...
function validateRecipients(recipients, dustAmount) {
  ...
  for (const recipient of recipients) {
    let address;
    try {
      address = chunk4QHMS5XH_cjs.ArkAddress.decode(recipient.address);
    } catch (e) {
...
var VtxoManager = class _VtxoManager {
  ...
  async sweepExpiredBoardingUtxos(prefetchedUtxos) {
    ...
    const sequence = chunk4QHMS5XH_cjs.getSequence(exitTapLeafScript);
  ...
  getBoardingTimelock() {
    ...
    const exitScript = chunk4QHMS5XH_cjs.CSVMultisigTapscript.decode(
  ...
  extractSpentOutpoint(error) {
    const ark = chunkISZA7V2J_cjs.maybeArkError(error);
  ...
  async runPeriodicSettle(boardingUtxos) {
    ...
    const outputFee = estimator.evalOffchainOutput({
      amount: totalAmount,
      script: base.hex.encode(chunk4QHMS5XH_cjs.ArkAddress.decode(arkAddress).pkScript)
    });
...
var ArkNote = class _ArkNote {
  ...
  constructor(preimage, value, HRP = _ArkNote.DefaultHRP) {
    ...
    this.vtxoScript = new chunk4QHMS5XH_cjs.VtxoScript([noteTapscript(preimageHash)]);
  ...
  static fromString(noteStr, hrp = _ArkNote.DefaultHRP) {
    ...
    const decoded = base.base58.decode(encoded);
    if (decoded.length === 0) {
   ...
   */
  toString() {
    return this.HRP + base.base58.encode(this.encode());
  }
};
...
var TxTree = class {
  ...
  validate() {
    ...
    for (const [outputIndex, child] of this.children) {
      ...
      if (!childInput.txid || base.hex.encode(childInput.txid) !== parentTxid || childInput.index !== outputIndex) {
...
function decodeNode(chunk) {
  const tx = transaction_js.Transaction.fromPSBT(base.base64.decode(chunk.tx));
    ...
    for await (const event of eventIterator) {
      ...
      switch (event.type) {
        ...
        case "tree_signature" /* TreeSignature */: {
          ...
          const tapKeySig = base.hex.decode(event.signature);
...
var AssetManager = class extends ReadonlyAssetManager {
  ...
  async issue(params) {
    ...
    const controlAssetRef = params.controlAssetId ? chunkISZA7V2J_cjs.AssetRef.fromId(chunkISZA7V2J_cjs.AssetId.fromString(params.controlAssetId)) : null;
    ...
    const groups = [];
    const issuedAssetOutput = chunkISZA7V2J_cjs.AssetOutput.create(0, params.amount);
    const issuedAssetGroup = chunkISZA7V2J_cjs.AssetGroup.create(
      null,
      controlAssetRef,
      [],
      [issuedAssetOutput],
      ...
      for (const [assetId, amount] of assetChanges) {
        ...
        for (const [inputIndex, assets] of assetInputs) {
          for (const asset of assets) {
            ...
            changeInputs.push(chunkISZA7V2J_cjs.AssetInput.create(inputIndex, asset.amount));
          }
        }
        groups.push(
          chunkISZA7V2J_cjs.AssetGroup.create(
            chunkISZA7V2J_cjs.AssetId.fromString(assetId),
            null,
            changeInputs,
            [chunkISZA7V2J_cjs.AssetOutput.create(0, amount)],
            []
          )
        );
      }
    ...
    const outputAddress = chunk4QHMS5XH_cjs.ArkAddress.decode(address);
    const outputs = [
      {
        script: outputAddress.pkScript,
        amount: BigInt(totalBtcSelected)
      },
      Extension.create([chunkISZA7V2J_cjs.Packet.create(groups)]).txOut()
    ];
    ...
    return {
      arkTxId: arkTxid,
      assetId: chunkISZA7V2J_cjs.AssetId.create(arkTxid, 0).toString()
    };
  }
  /**
  ...
  async reissue(params) {
    ...
    for (const [inputIndex, assets] of assetInputs) {
      for (const asset of assets) {
        ...
        reissueInputs.push(chunkISZA7V2J_cjs.AssetInput.create(inputIndex, asset.amount));
      }
    }
    const totalAssetAmount = assetToReissueAmount + params.amount;
    const reissueAssetIdObj = chunkISZA7V2J_cjs.AssetId.fromString(params.assetId);
    const reissueAssetGroup = chunkISZA7V2J_cjs.AssetGroup.create(
      reissueAssetIdObj,
      null,
      reissueInputs,
      [chunkISZA7V2J_cjs.AssetOutput.create(0, totalAssetAmount)],
      []
    );
    ...
    for (const [assetId, amount] of assetChanges) {
      ...
      groups.push(
        chunkISZA7V2J_cjs.AssetGroup.create(
          ...
          null,
          changeInputs,
          amount > 0n ? [chunkISZA7V2J_cjs.AssetOutput.create(0, amount)] : [],
          []
        )
      );
    }
...
function castMetadata(metadata) {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    ...
    md.push(chunkISZA7V2J_cjs.Metadata.create(textEncoder.encode(key), valueBytes));
  }
  return md;
}
var DelegateManagerImpl = class {
  ...
  async delegate(vtxos, destination, delegateAt) {
    ...
    const destinationScript = chunk4QHMS5XH_cjs.ArkAddress.decode(destination).pkScript;
...
async function delegate(identity, delegateProvider, arkInfo, delegateInfo, vtxos, destinationScript, delegateAt) {
  ...
  if (delegateFee > 0n) {
    outputs.push({
      script: chunk4QHMS5XH_cjs.ArkAddress.decode(delegateAddress).pkScript,
      amount: delegateFee
    });
  }
  ...
  const forfeitOutputScript = btcSigner.OutScript.encode(
    btcSigner.Address(chunk4QHMS5XH_cjs.getNetwork(network)).decode(forfeitAddress)
  );
...
async function makeSignedDelegateIntent(identity, coins, outputs, onchainOutputsIndexes, cosignerPubKeys, validAt, destinationScript) {
  ...
  const proof = chunkISZA7V2J_cjs.Intent.create(message, coins, outputs);
...
function findDestinationOutputIndex(outputs, destinationScript) {
  return outputs.findIndex((o) => o.script && utils_js.equalBytes(o.script, destinationScript));
}
...
function findDelegateTapLeaf(vtxo, delegatePubkey) {
  ...
  const vtxoScript = chunk4QHMS5XH_cjs.VtxoScript.decode(vtxo.tapTree);
  return vtxoScript.leaves.find((tapLeaf) => {
    const arkTapscript = chunk4QHMS5XH_cjs.decodeTapscript(chunk4QHMS5XH_cjs.scriptFromTapLeafScript(tapLeaf));
    if (!chunk4QHMS5XH_cjs.MultisigTapscript.is(arkTapscript)) return false;
...
function scriptFromArkAddress(address) {
  return base.hex.encode(chunk4QHMS5XH_cjs.ArkAddress.decode(address).pkScript);
}
  ...
  async connect(skipUpdate = false) {
    ...
    try {
      ...
      this.listenLoop().catch((e) => {
        if (chunkISZA7V2J_cjs.isEventSourceError(e)) {
...
var ContractManager = class _ContractManager {
  ...
  async upsertContract(params) {
    const handler = chunkJH7WWDEA_cjs.contractHandlers.get(params.type);
    if (!handler) {
      throw new Error(`No handler registered for contract type '${params.type}'`);
    }
    try {
      const script = handler.createScript(params.params);
      const derivedScript = base.hex.encode(script.pkScript);
  ...
  async scanContracts(opts) {
    ...
    const discoverables = chunkJH7WWDEA_cjs.contractHandlers.getRegisteredTypes().map((t) => chunkJH7WWDEA_cjs.contractHandlers.get(t)).filter(isDiscoverable);
...
var WalletReceiveRotator = class _WalletReceiveRotator {
  ...
  static async resolveBoot(config, setup) {
    ...
    const expectedContractType = setup.offchainTapscript instanceof chunkJH7WWDEA_cjs.DelegateVtxo.Script ? "delegate" : "default";
    const factoryOpts = {
    ...
    const offchainTapscript = utils_js.equalBytes(
      boot.receivePubkey,
  ...
  async rotate(wallet) {
    ...
    const manager = await wallet.getContractManager();
    const csvTimelock = newTapscript.options.csvTimelock ?? chunkJH7WWDEA_cjs.DefaultVtxo.Script.DEFAULT_TIMELOCK;
    const csvTimelockStr = chunk4QHMS5XH_cjs.timelockToSequence(csvTimelock).toString();
    ...
    const baseParams = {
      ...
      metadata: {
        source: chunkJH7WWDEA_cjs.WALLET_RECEIVE_SOURCE,
        signingDescriptor: descriptor
      }
    };
    if (newTapscript instanceof chunkJH7WWDEA_cjs.DelegateVtxo.Script) {
...
function deriveLeafPubkey(descriptor) {
  try {
    return chunkJH7WWDEA_cjs.deriveDescriptorLeafPubKey(descriptor);
  } catch (e) {
...
function rebuildTapscript(current, pubKey) {
  if (current instanceof chunkJH7WWDEA_cjs.DelegateVtxo.Script) {
    return new chunkJH7WWDEA_cjs.DelegateVtxo.Script({ ...current.options, pubKey });
  }
  return new chunkJH7WWDEA_cjs.DefaultVtxo.Script({ ...current.options, pubKey });
}
async function pickActiveReceive(contractRepository, serverPubKey, expectedType) {
  ...
  const matching = candidates.filter(
    (c) => c.params.serverPubKey === serverPubKeyHex && c.metadata?.source === chunkJH7WWDEA_cjs.WALLET_RECEIVE_SOURCE
  ).sort((a, b) => {
  ...
  try {
    return {
      pubKey: base.hex.decode(newest.params.pubKey),
...
var getArkadeServerUrl = ({ arkServerUrl }) => arkServerUrl || chunk4QHMS5XH_cjs.DEFAULT_ARKADE_SERVER_URL;
function intentProofJobs(coins) {
  if (coins.length === 0) return [];
  const coinJobs = coins.map((coin, i) => ({
    index: i + 1,
    lookupScript: chunk4QHMS5XH_cjs.VtxoScript.decode(coin.tapTree).pkScript
  }));
...
function dedupeTimelocks(timelocks) {
  ...
  for (const timelock of timelocks) {
    const sequence = chunk4QHMS5XH_cjs.timelockToSequence(timelock).toString();
...
var ReadonlyWallet = class _ReadonlyWallet {
  constructor(identity, network, onchainProvider, indexerProvider, arkServerPublicKey, offchainTapscript, boardingTapscript, dustAmount, walletRepository, contractRepository, delegateProvider, watcherConfig, walletContractTimelocks) {
    ...
    this.walletContractTimelocks = walletContractTimelocks && walletContractTimelocks.length > 0 ? dedupeTimelocks(walletContractTimelocks) : [
      this.offchainTapscript.options.csvTimelock ?? chunkJH7WWDEA_cjs.DefaultVtxo.Script.DEFAULT_TIMELOCK
    ];
  }
  _contractManager;
  ...
  static async setupWalletConfig(config, pubKey) {
    ...
    const arkProvider = config.arkProvider || new chunkISZA7V2J_cjs.RestArkProvider(arkadeServerUrl);
    ...
    if (!indexerProvider) {
      ...
      indexerProvider = new chunkISZA7V2J_cjs.RestIndexerProvider(indexerUrl);
    }
    const info = await arkProvider.getInfo();
    const network = chunk4QHMS5XH_cjs.getNetwork(info.network);
    ...
    const offchainTapscript = !delegatePubKey ? new chunkJH7WWDEA_cjs.DefaultVtxo.Script(offchainOptions) : new chunkJH7WWDEA_cjs.DelegateVtxo.Script({ ...offchainOptions, delegatePubKey });
    const boardingTapscript = new chunkJH7WWDEA_cjs.DefaultVtxo.Script({
      ...offchainOptions,
  ...
  get defaultContractScript() {
    return base.hex.encode(this.offchainTapscript.pkScript);
  }
  ...
  async initializeContractManager() {
    ...
    for (const csvTimelock of this.walletContractTimelocks) {
      const csvTimelockStr = chunk4QHMS5XH_cjs.timelockToSequence(csvTimelock).toString();
      const defaultScript = new chunkJH7WWDEA_cjs.DefaultVtxo.Script({
      ...
      if (this.offchainTapscript instanceof chunkJH7WWDEA_cjs.DelegateVtxo.Script) {
        const delegateScript = new chunkJH7WWDEA_cjs.DelegateVtxo.Script({
          pubKey: baselinePubkey,
          serverPubKey: this.offchainTapscript.options.serverPubKey,
...
var Wallet2 = class _Wallet extends ReadonlyWallet {
  ...
  async _runRestore(gapLimit) {
    ...
    const delegatePubKey = this.offchainTapscript instanceof chunkJH7WWDEA_cjs.DelegateVtxo.Script ? this.offchainTapscript.options.delegatePubKey : void 0;
    const deps = {
      indexerProvider: this.indexerProvider,
  ...
  static async create(config) {
    ...
    try {
      ...
      serverUnrollScript = chunk4QHMS5XH_cjs.CSVMultisigTapscript.decode(raw);
    } catch (e) {
  ...
  async sendBitcoin(params) {
    ...
    if (params.selectedVtxos && params.selectedVtxos.length > 0) {
      return this._withTxLock(async () => {
        ...
        const outputAddress = chunk4QHMS5XH_cjs.ArkAddress.decode(params.address);
  ...
  async _settleImpl(params, eventCallback) {
    ...
    if (!params) {
      ...
      const outputFee = estimator.evalOffchainOutput({
        amount: output.amount,
        script: base.hex.encode(chunk4QHMS5XH_cjs.ArkAddress.decode(output.address).pkScript)
      });
    ...
    for (const [index, output] of params.outputs.entries()) {
      let script;
      try {
        const addr = chunk4QHMS5XH_cjs.ArkAddress.decode(output.address);
    ...
    let outputAssets;
    const destinationScript = chunk4QHMS5XH_cjs.ArkAddress.decode(await this.getAddress()).pkScript;
    const assetOutputIndex = findDestinationOutputIndex(outputs, destinationScript);
    if (assetInputs.size > 0) {
      if (assetOutputIndex === -1) {
        throw new Error("Cannot assign assets: no output matches the destination address");
    ...
    if (hasOffchainOutputs) {
      ...
      signingPublicKeys.push(base.hex.encode(await session.getPublicKey()));
    }
  ...
  async handleSettlementFinalizationEvent(event, inputs, forfeitOutputScript, connectorsGraph) {
    ...
    for (const input of inputs) {
      if (!isVtxo(input)) {
        for (let i = 0; i < settlementPsbt.inputsLength; i++) {
          ...
          const inputTxId = base.hex.encode(settlementInput.txid);
    ...
    if (signedForfeits.length > 0 || hasBoardingUtxos) {
      await this.arkProvider.submitSignedForfeitTxs(
        signedForfeits,
        hasBoardingUtxos ? base.base64.encode(settlementPsbt.toPSBT()) : void 0
      );
    }
  }
  /**
  ...
  createBatchHandler(intentId, inputs, expectedRecipients, session) {
    ...
    return {
      onBatchStarted: async (event) => {
        ...
        const sweepTapscript = chunk4QHMS5XH_cjs.CSVMultisigTapscript.encode({
          timelock: {
      ...
      onTreeSigningStarted: async (event, vtxoTree) => {
        ...
        const commitmentTx = btcSigner.Transaction.fromPSBT(
          base.base64.decode(event.unsignedCommitmentTx)
        );
  ...
  async safeRegisterIntent(intent, inputs) {
    try {
      ...
      if (error instanceof chunkISZA7V2J_cjs.ArkError && error.code === 0 && error.message.includes("duplicated input")) {
  ...
  async makeDeleteIntentSignature(coins) {
    ...
    const proof = chunkISZA7V2J_cjs.Intent.create(message, coins, []);
    const signedProof = await this._signerRouter.sign(proof, intentProofJobs(coins));
    return {
      proof: base.base64.encode(signedProof.toPSBT()),
      message
    ...
    try {
      ...
      if (changeAmount > 0n && batchExpiry !== Number.MAX_SAFE_INTEGER) {
        changeVtxo = {
          ...
          assets: changeAssets,
          script: base.hex.encode(offchainTapscript.pkScript)
        };
      }
...
var MessageBus = class {
  ...
  async buildServices(config) {
    const arkProvider = new chunkISZA7V2J_cjs.RestArkProvider(config.arkServer.url);
    const storage = {
      walletRepository: this.walletRepository,
      contractRepository: this.contractRepository
    };
...
var Ramps = class {
  ...
  async onboard(feeInfo, boardingUtxos, amount, eventCallback) {
    ...
    const offchainAddr = chunk4QHMS5XH_cjs.ArkAddress.decode(offchainAddress);
  ...
  async offboard(destinationAddress, feeInfo, amount, eventCallback) {
    ...
    for (const networkName of networkNames) {
      try {
        const network = chunk4QHMS5XH_cjs.networks[networkName];
    ...
    const outputFee = estimator.evalOnchainOutput({
      amount,
      script: base.hex.encode(destinationScript)
    });
    if (BigInt(outputFee.satoshis) > amount) {
      throw new Error(
...
var WalletMessageHandler = class {
  ...
  async handleInitWallet({ payload }) {
    ...
    this.indexerProvider = new chunkISZA7V2J_cjs.RestIndexerProvider(arkServerUrl);
...
var ServiceWorkerReadonlyWallet = class _ServiceWorkerReadonlyWallet {
  ...
  static async create(options) {
    ...
    const publicKey = await options.identity.compressedPublicKey().then(base.hex.encode);
...
var OnchainWallet = class _OnchainWallet {
   ...
   */
  static async create(identity, networkName = chunk4QHMS5XH_cjs.DEFAULT_NETWORK_NAME, provider) {
    const pubkey = await identity.xOnlyPublicKey();
    if (!pubkey) {
      throw new Error("Invalid configured public key");
    }
    const network = chunk4QHMS5XH_cjs.getNetwork(networkName);
  ...
  async send(params) {
    ...
    let tx = new chunkISZA7V2J_cjs.Transaction();
    for (const input of inputs) {
      tx.addInput({
        txid: input.txid,
        index: input.vout,
  ...
  async bumpP2A(parent) {
    ...
    let child = new chunkISZA7V2J_cjs.Transaction({
      version: 3,
...
function parseBlockHeader(headerHex) {
  const headerBytes = base.hex.decode(headerHex);
  ...
  const hash1 = utils_js.sha256(headerBytes);
  const hash2 = utils_js.sha256(hash1);
  const hashStr = base.hex.encode(new Uint8Array(hash2).reverse());
...
var WsElectrumChainSource = class {
  ...
  waitForAddressReceivesTx(addr) {
    return new Promise((resolve, reject) => {
      const script = btcSigner.OutScript.encode(btcSigner.Address(this.network).decode(addr));
  ...
  async listUnspents(addr) {
    ...
    return unspentsFromElectrum.map((u, index) => {
      const tx = btcSigner.Transaction.fromRaw(base.hex.decode(txs[index].hex), {
  ...
  addressForScript(scriptHex) {
    try {
      const script = base.hex.decode(scriptHex);
      return btcSigner.Address(this.network).encode(btcSigner.OutScript.decode(script));
    } catch {
      return void 0;
    }
  }
...
var ElectrumOnchainProvider = class {
    ...
    if (ambiguousIndices.length > 0) {
      ...
      for (let j = 0; j < ambiguousIndices.length; j++) {
        ...
        for (const candidateTxid of ambiguousCandidates[j]) {
          ...
          if (!rawHex) continue;
          const candidateTx = btcSigner.Transaction.fromRaw(base.hex.decode(rawHex), {
            allowUnknownOutputs: true,
          ...
          for (let k = 0; k < candidateTx.inputsLength; k++) {
            ...
            if (input.txid && base.hex.encode(input.txid) === txid && input.index === i) {
  ...
  buildExplorerTx(entry, rawHex, blockTimeByHeight) {
    ...
    if (rawHex) {
      let tx;
      try {
        tx = btcSigner.Transaction.fromRaw(base.hex.decode(rawHex), {
          allowUnknownOutputs: true,
          allowUnknownInputs: true
        });
      } catch (err) {
      ...
      for (let i = 0; i < tx.outputsLength; i++) {
        const output = tx.getOutput(i);
        const scriptHex = output.script ? base.hex.encode(output.script) : "";
        vout.push({
  ...
  encodeAddress(address) {
    try {
      return btcSigner.OutScript.encode(btcSigner.Address(this.network).decode(address));
    } catch (err) {
...
function toScriptHash(script) {
  return base.hex.encode(utils_js.sha256(script).reverse());
}
...
((BIP3222) => {
  async function sign2(message, identity, network) {
    ...
    const toSpend = chunkISZA7V2J_cjs.craftToSpendTx(message, payment.script, TAG_BIP322);
    ...
    return base.base64.encode(btcSigner.RawWitness.encode(input.finalScriptWitness));
  }
  BIP3222.sign = sign2;
  function verify(message, signature, address, network) {
    let decoded;
    try {
      decoded = btcSigner.Address(network).decode(address);
    } catch {
      return false;
    }
    if (decoded.type === "pkh") {
      try {
        return verifyLegacy(message, base.base64.decode(signature), decoded.hash);
      } catch {
        return false;
      }
    }
    ...
    let witnessItems;
    try {
      pkScript = btcSigner.OutScript.encode(decoded);
      witnessItems = btcSigner.RawWitness.decode(base.base64.decode(signature));
    } catch {
      return false;
    }
...
function verifyP2TR(message, witnessItems, pkScript, pubkey) {
  ...
  const sighashType = sig.length === 65 ? sig[64] : btcSigner.SigHash.DEFAULT;
  if (sighashType !== btcSigner.SigHash.DEFAULT && sighashType !== btcSigner.SigHash.ALL) {
    return false;
  }
  const toSpend = chunkISZA7V2J_cjs.craftToSpendTx(message, pkScript, TAG_BIP322);
  ...
  return secp256k1_js.schnorr.verify(rawSig, sighash, pubkey);
}
function verifyP2WPKH(message, witnessItems, pkScript, addressHash) {
  ...
  const derived = btcSigner.p2wpkh(pubkey);
  if (!utils_js$1.equalBytes(derived.hash, addressHash)) {
    return false;
  }
  ...
  return secp256k1_js.secp256k1.verify(derSig, sighash, pubkey, {
    prehash: false,
    format: "der"
  });
}
function verifyLegacy(message, sigBytes, addressHash) {
  ...
  try {
    const sig = secp256k1_js.secp256k1.Signature.fromBytes(compactSig, "compact").addRecoveryBit(recoveryId);
    const point = sig.recoverPublicKey(msgHash);
    const pubkeyBytes = point.toBytes(compressed);
    return utils_js$1.equalBytes(utils_js.hash160(pubkeyBytes), addressHash);
  } catch {
    return false;
  }
}
function bitcoinMessageHash(message) {
  ...
  return utils_js.sha256x2(utils_js.concatBytes(MAGIC, encodeCompactSize(msgBytes.length), msgBytes));
}
...
function craftBIP322ToSignP2TR(toSpend, pkScript, tapInternalKey) {
  const tx = new chunkISZA7V2J_cjs.Transaction({ version: 0 });
  tx.addInput({
    txid: toSpend.id,
    index: 0,
    sequence: 0,
  ...
  tx.addOutput({
    amount: 0n,
    script: chunkISZA7V2J_cjs.OP_RETURN_EMPTY_PKSCRIPT
  });
  return tx;
}
...
((Unroll2) => {
  ...
  class Session2 {
    ...
    async next() {
      ...
      const tx = chunkISZA7V2J_cjs.Transaction.fromPSBT(base.base64.decode(virtualTxs.txs[0]));
...
async function prepareUnrollTransaction(wallet, vtxoTxIds, outputAddress) {
  ...
  for (const vtxo of vtxos) {
    ...
    const spendingLeaf = chunk4QHMS5XH_cjs.VtxoScript.decode(vtxo.tapTree).findLeaf(base.hex.encode(exit.script));
    if (!spendingLeaf) {
      throw new Error(`spending leaf not found for vtxo ${vtxo.txid}:${vtxo.vout}`);
    }
    totalAmount += BigInt(vtxo.value);
    const sequence = chunk4QHMS5XH_cjs.timelockToSequence(exit.params.timelock);
    inputs.push({
      txid: vtxo.txid,
      index: vtxo.vout,
      ...
      witnessUtxo: {
        amount: BigInt(vtxo.value),
        script: chunk4QHMS5XH_cjs.VtxoScript.decode(vtxo.tapTree).pkScript
      },
      sighashType: btcSigner.SigHash.DEFAULT
    });
  ...
  const tx = new chunkISZA7V2J_cjs.Transaction({ version: 2 });
  for (const input of inputs) {
    tx.addInput(input);
  }
...
function availableExitPath(confirmedAt, current, vtxo) {
  const exits = chunk4QHMS5XH_cjs.VtxoScript.decode(vtxo.tapTree).exitPaths();
...
function computeArkadeScriptPublicKey(pubKey, script) {
  ...
  const point = secp256k1_js.secp256k1.Point.fromHex("02" + base.hex.encode(xOnly));
  const scalar = bytesToBigInt(hash) % secp256k1_js.secp256k1.Point.CURVE().n || 1n;
  const tweakPoint = secp256k1_js.secp256k1.Point.BASE.multiply(scalar);
...
function createArkadeBatchHandler(intentId, inputs, signer, signedProof, message, session, arkProvider, emulator, network) {
  ...
  return {
    ...
    onTreeSigningStarted: async (event, vtxoTree) => {
      ...
      const commitmentTx = chunkISZA7V2J_cjs.Transaction.fromPSBT(base.base64.decode(event.unsignedCommitmentTx));
      const sharedOutput = commitmentTx.getOutput(0);
      if (!sharedOutput?.amount) {
        throw new Error("Shared output not found");
      }
    ...
    onBatchFinalization: async (event, _vtxoTree, connectorTree) => {
      ...
      let commitmentPsbt = chunkISZA7V2J_cjs.Transaction.fromPSBT(base.base64.decode(event.commitmentTx));
      const signedForfeits = [];
...
function reEncodeTapscript(tapscript) {
  switch (tapscript.type) {
    case "multisig" /* Multisig */:
      return chunk4QHMS5XH_cjs.MultisigTapscript.encode(tapscript.params).script;
    case "csv-multisig" /* CSVMultisig */:
      return chunk4QHMS5XH_cjs.CSVMultisigTapscript.encode(tapscript.params).script;
    case "condition-csv-multisig" /* ConditionCSVMultisig */:
      return chunk4QHMS5XH_cjs.ConditionCSVMultisigTapscript.encode(tapscript.params).script;
    case "condition-multisig" /* ConditionMultisig */:
      return chunk4QHMS5XH_cjs.ConditionMultisigTapscript.encode(tapscript.params).script;
    case "cltv-multisig" /* CLTVMultisig */:
      return chunk4QHMS5XH_cjs.CLTVMultisigTapscript.encode(tapscript.params).script;
    default:
...
var ArkadeVtxoScript = class extends chunk4QHMS5XH_cjs.VtxoScript {
  arkadeScripts;
  constructor(scripts) {
...
function contractFromArkContract(encoded, options = {}) {
  ...
  const handler = chunkJH7WWDEA_cjs.contractHandlers.get(parsed.type);
  if (!handler) {
...
function contractFromArkContractWithAddress(encoded, serverPubKey, addressPrefix = chunk4QHMS5XH_cjs.DEFAULT_NETWORK.hrp, options = {}) {
  const parsed = decodeArkContract(encoded);
  const handler = chunkJH7WWDEA_cjs.contractHandlers.getOrThrow(parsed.type);
  const params = parsed.data;
  ...
  return {
    ...
    type: parsed.type,
    params,
    script: base.hex.encode(vtxoScript.pkScript),
