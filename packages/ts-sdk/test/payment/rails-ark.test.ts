...
describe("arkRail", () => {
    it("matches a bare ark address and the ark= param of a BIP21 URI", () => {
        const r = arkRail();
        expect(r.match({ raw: arkAddr }, ctx())).toBe(true);
        expect(r.match({ raw: `bitcoin:bcrt1qexample?ark=${arkAddr}` }, ctx())).toBe(true);
    });
    ...
    it("quote.send() calls wallet.send and settles with a txid result", async () => {
        ...
        const q = await arkRail().quote({ raw: arkAddr, amount: 1000 }, ctx(send));
    ...
    it("falls back to the BIP21-encoded amount (BTC) converted to sats", async () => {
        ...
        const q = await arkRail().quote(
            { raw: `bitcoin:?ark=${arkAddr}&amount=0.0001` },
            ctx(send),
        );
    ...
    it("rejects a missing, zero, or fractional amount at quote time", async () => {
        ...
        await expect(arkRail().quote({ raw: arkAddr, amount: 0 }, ctx())).rejects.toThrow(
            /invalid amount/i,
        );
        await expect(arkRail().quote({ raw: arkAddr, amount: 1.5 }, ctx())).rejects.toThrow(
            /invalid amount/i,
        );
    });
});
