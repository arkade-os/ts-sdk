...
// The indexer returns every VTXO (including spent ones) when queried without
// `spendableOnly`, and convertVtxo now classifies spent VTXOs with a distinct
// `virtualStatus.state === "spent"`. These paths consume the raw indexer stream
// and must exclude spent VTXOs via isSpent, not rely on a state denylist.
describe("Wallet pending-tx recovery excludes spent VTXOs", () => {
    const mockPrivKeyHex = "ce66c68f8875c0c98a502c666303dc183a21600130013c06f9d1edf60207abf2";
    const mockServerKeyHex = "0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798";
    const mockIdentity = SingleKey.fromHex(mockPrivKeyHex);
    ...
    // Route bootstrap fetches by URL so ordering between the foreground
    // Promise.all and the background VtxoManager init doesn't matter.
    function bootstrapMock() {
        mockFetch.mockImplementation((url: string) => {
            if (url.includes("/v1/info")) {
                return Promise.resolve({ ok: true, json: () => Promise.resolve(mockArkInfo) });
            }
            if (url.includes("/script/subscribe")) {
                return Promise.resolve({
                    ok: true,
            ...
            if (url.includes("/vtxos")) {
                return Promise.resolve({ ok: true, json: () => Promise.resolve({ vtxos: [] }) });
            }
            return Promise.resolve({
                ok: true,
                json: () => Promise.resolve({}),
    ...
    async function createWallet() {
        ...
        // Stop background VtxoManager work so it can't race the spies below.
        await wallet.dispose();
        return wallet;
    }

    it("fetchPendingTxs drops spent VTXOs", async () => {
        const wallet = await createWallet();

        const preconfirmed = createMockVtxo({
            txid: "aa".repeat(32),
            virtualStatus: { state: "preconfirmed" },
            isSpent: false,
            arkTxId: "arktx-preconfirmed",
        });
        ...
        vi.spyOn(wallet, "getWalletScripts").mockResolvedValue(["scripthex"]);
        vi.spyOn(wallet.indexerProvider, "getVtxos").mockResolvedValue({
            vtxos: [preconfirmed, spent],
        });

        const pending = await wallet.fetchPendingTxs();

        // Only the preconfirmed VTXO is genuinely pending; the spent one must
        // not be reported even though its state is no longer "settled".
        expect(pending).toEqual(["arktx-preconfirmed"]);
    });

    it("finalizePendingTxs excludes spent VTXOs from the recovery batch", async () => {
        const wallet = await createWallet();

        const script = "scripthex";
        const fakeVtxoScript = {
            forfeit: () => [new Uint8Array(32), new Uint8Array(33)],
            encode: () => new Uint8Array(64),
        };
        const preconfirmed = createMockVtxo({
        ...
        vi.spyOn(wallet as any, "hasPendingTxFlag").mockResolvedValue(true);
        vi.spyOn(wallet, "getScriptMap").mockResolvedValue(
            new Map([[script, fakeVtxoScript as any]]),
        );
        vi.spyOn(wallet.indexerProvider, "getVtxos").mockResolvedValue({
            vtxos: [preconfirmed, spent],
        });
        const intentSpy = vi
            .spyOn(wallet, "makeGetPendingTxIntentSignature")
            .mockResolvedValue({ proof: "proof", message: "message" } as any);
        vi.spyOn(wallet.arkProvider as any, "getPendingTxs").mockResolvedValue([]);

        await wallet.finalizePendingTxs();

        expect(intentSpy).toHaveBeenCalledOnce();
        const batch = intentSpy.mock.calls[0][0];
        const txids = batch.map((v) => v.txid);
        expect(txids).toContain("aa".repeat(32));
        expect(txids).not.toContain("dd".repeat(32));
    });
});
