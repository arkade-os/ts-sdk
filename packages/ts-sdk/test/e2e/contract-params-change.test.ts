...
describe("Contract params change", () => {
    ...
    it("registers new default + delegate contracts after server exit delay change and keeps receiving on old and new addresses", {
        timeout: 120000,
    }, async () => {
        ...
        try {
            ...
            expect(vtxosAfterFirstFund.some((v) => v.value === fundAmountOld)).toBe(true);
            ...
            const newDelegate = delegateContracts.find((c) => c.script !== oldDelegate!.script);
            const newDefault = defaultContracts.find((c) => c.script !== oldDefault!.script);
            ...
            expect(newDelegate!.params.csvTimelock).not.toBe(oldDelegateCsv);
            expect(newDefault!.params.csvTimelock).not.toBe(oldDefaultCsv);
            expect(newDelegate!.params.csvTimelock).toBe(newDefault!.params.csvTimelock);
            ...
            expect(persistedOldDelegate!.params.csvTimelock).toBe(oldDelegateCsv);
            expect(persistedOldDefault!.params.csvTimelock).toBe(oldDefaultCsv);

            // Old VTXO must still be visible after reload (the repository kept it).
            const vtxosAfterReload = await second.wallet.getVtxos();
            expect(vtxosAfterReload.some((v) => v.value === fundAmountOld)).toBe(true);
            ...
            expect(finalVtxos.some((v) => v.value === fundAmountOld)).toBe(true);
            expect(finalVtxos.some((v) => v.value === fundAmountNew)).toBe(true);
