...
describe("vhtlc", () => {
    ...
    it("should claim with a preimage re-derived from a static wallet's seed and salt", {
        timeout: 60000,
    }, async () => {
        ...
        expect(hex.encode(hash160(preimage))).toBe(hex.encode(vhtlcScript.options.preimageHash));
        ...
        const claimIdentity: Identity = {
            ...
            signMessage: (message, signatureType) => restored.signMessage(message, signatureType),
        };
    ...
    it("should give two swaps on one static wallet unrelated covenants", {
        timeout: 60000,
    }, async () => {
    ...
    it("should refund without receiver on a post-maturity retry of a rejected CLTV tx", {
        timeout: 120_000,
    }, async () => {
        const alice = createTestIdentity();
        const bob = createTestIdentity();
        ...
        const rejection = await arkProvider
            ...
            .then(
                () => {
                    throw new Error("submitTx accepted a CLTV spend before its locktime matured");
                },
                (e: unknown) => e,
            );
