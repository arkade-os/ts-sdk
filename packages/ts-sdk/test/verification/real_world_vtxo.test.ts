...
describe("Production Audit: Signet-Structured VTXO Validation", () => {
    ...
    it("should verify a 3-depth chain matching signet Ark structure", async () => {
        ...
        const vtxoLeaf = createVirtualTx(
            treeNode.txid,
            0,
        ...
        signVirtualTx(vtxoLeaf.tx, 0, TEST_PRIVKEYS[1], [
            { script: makeP2TRScript(1), amount: 250000n },
        ]);
        ...
        const indexer: VerificationIndexerProvider = {
            ...
            getVirtualTxs: vi.fn().mockResolvedValue({
                txs: [base64.encode(vtxoLeaf.tx.toPSBT()), base64.encode(treeNode.tx.toPSBT())],
            }),
        };
        ...
        const onchain: VerificationOnchainProvider = {
            ...
            getTxStatus: vi
                .fn()
                .mockResolvedValue({ confirmed: true, blockHeight: 200000, blockTime: 1700000000 }),
    ...
    it("should verify a chain with checkpoint transaction", async () => {
        ...
        signVirtualTx(checkpoint.tx, 0, TEST_PRIVKEYS[0], [
            { script: rootScript, amount: 100000n },
        ]);

        // VTXO spending from checkpoint
        const vtxo = createVirtualTx(
            checkpoint.txid,
            0,
        ...
        signVirtualTx(vtxo.tx, 0, TEST_PRIVKEYS[1], [
        ...
        const indexer: VerificationIndexerProvider = {
            ...
            getVirtualTxs: vi.fn().mockResolvedValue({
                txs: [base64.encode(vtxo.tx.toPSBT()), base64.encode(checkpoint.tx.toPSBT())],
            }),
        };
