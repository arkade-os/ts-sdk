...
describe("ArkadeVtxoScript", () => {
    ...
    it("should produce correct tweaked pubkey in multisig leaf", () => {
        ...
        expect(hex.encode(vtxo.tweakedPublicKey)).toBe(hex.encode(manualVtxo.tweakedPublicKey));
    });
    ...
    it("should store arkade script bytes in arkadeScripts map", () => {
        ...
        expect(hex.encode(vtxo.arkadeScripts.get(0)!)).toBe(hex.encode(arkadeScriptBytes));
    });
    ...
    it("should match manual VtxoScript with arkade multisig + CSV exit", () => {
        ...
        const manualVtxo = new VtxoScript([manualMultisig.script, csvExit.script]);
        expect(hex.encode(vtxo.tweakedPublicKey)).toBe(hex.encode(manualVtxo.tweakedPublicKey));
    ...
    it("should support multiple arkade leaves", () => {
        const arkadeScript2 = ArkadeScript.encode(["INSPECTNUMASSETGROUPS", 1, "EQUAL"]);
        ...
        expect(hex.encode(vtxo.arkadeScripts.get(0)!)).toBe(hex.encode(arkadeScriptBytes));
        expect(hex.encode(vtxo.arkadeScripts.get(1)!)).toBe(hex.encode(arkadeScript2));
    });
