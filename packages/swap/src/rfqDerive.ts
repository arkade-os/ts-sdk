/**
 * Corridor derivation functions for RFQ swaps.
 *
 * Extracted from rfq.ts — pure derivation logic that builds local contracts
 * from quote fields and trader-owned data. No transport or comms logic.
 */
...
import { AddressMismatch } from "./rfqErrors";
import { verifyLockupAddress } from "./rfqVerify";
import type { RfqQuote } from "./rfq";

/** Decode a solver-supplied hex field — shared with rfq.ts but kept local to
 * avoid a circular import on a pure utility. */
