...
export async function restoreAssetSwaps(
    ...
    for (const { fundingTx, offer, offerHex, existing } of found) {
        ...
        const spentTxid = vtxo.isSpent === true ? vtxo.arkTxId || vtxo.spentBy : undefined;
        let status: AssetSwapStatus = "pending";
        if (vtxo.isSpent !== true && vtxo.isSwept === true) status = "recoverable";
        else if (vtxo.isSpent === true) {
