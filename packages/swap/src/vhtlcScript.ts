/**
 * VHTLC script builders and timing helpers for the RFQ corridors.
 *
 * `lightningSendVtxoScript` is the eight-leaf send-direction tree; `receiveVtxoScript`
 * is the same tree with roles inverted for the receive corridors. The timing helpers
 * (`unilateralClaimDelay`, etc.) are pure derivations from the Ark server's exit
 * delay — both client and solver read the same server, so deriving rather than
 * quoting is what keeps the two scripts identical.
 */
...
export function lightningSendVtxoScript(params: {
     ...
     * (see `requestLightningSend`'s own obligations). */
...
export function receiveVtxoScript(params: {
    ...
    /** From {@link unilateralClaimDelay} over the trader's OWN server info. */
    claimDelay: number;
    /** Emulator x-only key — see `requestLightningSend`'s parameter. */
    emulatorPubkey: Uint8Array;
