...
Decision: the public API uses one CAIP-19 BTC id across corridors — an intentional API simplification, not direct NArk compatibility by type shape (NArk models Ark, Lightning, and onchain BTC as distinct corridor assets). A registry-backed alias layer in the swap package canonicalizes caller input and maps public CAIP-19 endpoint ids back to the discovery/RFQ vocabulary; it is a compatibility adapter, never caller API. The rules: ticker aliases (`"BTC"`, `"USDT"`) are case-insensitive and network-scoped, and an alias that collides on a network is rejected rather than guessed; the single BTC id maps to each corridor leg's discovery name (`btc`) and to the protocol pair strings (for example `arkade:BTC->lightning:BTC`); an `arkade:<genesis_txid>.<idx>` id maps to the covenant/TLV identity form, which must round-trip through the alias layer byte-for-byte (the shared vectors in `test/offer.test.ts` and `test/rfq.test.ts` pin the endianness rules). Registry data comes from discovery by default and accepts injected overrides.
...
Storage. Browser defaults to IndexedDB. Node defaults for real swap operation to file-backed SQLite, reusing `packages/swap/src/repositories/sqlite/repository.ts`; the `node:sqlite` executor follows `config/test-helpers/nodeSqlExecutor.ts` with a file path rather than `:memory:`. The default path lives in the platform config dir (XDG / `~/Library/Application Support` / `%APPDATA%`) under `arkade/swaps/swaps-<network>.sqlite`. Ownership: the client closes connections it created on `stop()`/dispose; injected repositories are caller-owned and the client never closes them. In-memory storage remains available only through an explicit ephemeral/test mode.

`client.ready` failure semantics. `ready` rejects only when the repository itself is unreadable: a client that cannot read its own records cannot drive them safely, so nothing proceeds from there. Everything per-record or per-swap resolves `ready` and surfaces through the normal channels instead — a corrupt record is filtered rather than fatal (the v1 store's rule), and anything it protected is recoverable through the wallet's VTXO recovery plus `client.recover`; a first restore pass that produces `needs_recovery` surfaces those swaps through `swaps()` and `onUpdate`; a missing corridor dependency is `MissingCorridorDep` when a quote touches that corridor (§6), never a construction failure. `drive:"readonly"` follows the same rules with no actuation.
...
Resolution is also available without new disclosure: `client.resolve(input)` performs the same parsing and returns the resolved route when possible, so application policy can veto before an RFQ round trip discloses an invoice or an amount. Market and solver selection are network-free only against injected or cached discovery data. If no discovery snapshot is available, `resolve()` throws `DiscoverySnapshotUnavailable`: an unresolved route shape would be a third route state the type system deliberately excludes, and a caller that needs offline resolution can inject or warm a snapshot. `quote()` runs the same veto internally through `policy.selectMarket`, may fetch discovery as part of the quote path, and every `Quote` carries its market provenance for audit.
...
One call, one contract: persist the full record and secrets first, then fund, then update the record with the funding txid. `accept(quote)` is idempotent by **quote id, and quote id only**: the client mints `Quote.id` at quote time (feed-priced offer quotes have no solver-minted id, so the client mints everywhere), and deduping on a deterministic content hash is a policy concern, not identity. Once an accept record exists, later calls with the same quote return or resume that record; they must not create a new swap, a new invoice, or a second funding attempt. `AcceptConflict` compares the persisted record against the incoming quote field by field — route pair, both assets and amounts, instrument, lock hash, `refundLocktime`, solver/registry — and only differing material fields are incompatible; the funding txid being set where it was absent is a benign resume, never a conflict. If funding may have happened but the txid is unknown, recovery reconciles from wallet/indexer/contract evidence before any funding retry.
...
The built-in BOLT11 decoder is `light-bolt11-decoder`, a dependency of the swap package — core deliberately carries no bolt11 dependency, and this matches what `@arkade-os/boltz-swap` already ships. It sits behind the lightning corridor's overridable `decode` dep, so integrators may substitute their own. Validation at the parse boundary: the hrp network prefix must correspond to the wallet's bitcoin network (`lnbc` / `lntb` / `lntbs` / `lnbcrt`); the payment hash is extracted and carried on the invoice instrument; an expired invoice, or one whose remaining validity leaves less than the corridor's claim headroom, is rejected (the gates v1's `assertFundable` applied); an amountless invoice is rejected on send routes, where the invoice is the amount pin — `amountOn` does not rescue it. Amountless instruments stay reserved for future reusable-instrument corridors (BOLT12), which define their own amount rules.
...
The asset id layout is already CAIP-19 with room reserved: `eip155` assets imply their own corridor, arkade assets live under bitcoin's `bip122` chain id, and BTC is a single id on every corridor. New networks are new corridor modules and new `Route` variants, not new client shapes; §9 is the worked instance. The alias map and its rules are specified in §2, including the deliberate divergence from NArk's corridor-specific BTC asset model.
