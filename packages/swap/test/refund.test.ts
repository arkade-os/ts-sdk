...
describe("pushRefundWithoutReceiver", () => {
    ...
    describe("swept outputs", () => {
         ...
         * sender key does not change that. The swept output has to be recovered
         * into a fresh batch instead of spent with an offchain transaction.
         */
...
describe("findLockupVtxos", () => {
    it("reads the lockup's registered contract and returns every unspent output", async () => {
        ...
        expect(await findLockupVtxos(contracts, script.pkScript)).toHaveLength(2);
        expect(contracts.asked[0]).toBe(hex.encode(script.pkScript));
    });
    ...
    it("counts an output appearing in both sets exactly once", async () => {
        ...
        const found = await findLockupVtxos(fakeContracts([both, both]), script.pkScript);
