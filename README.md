...
Releases run from the repository root and are package-scoped (`sdk`, `swap`, or `all`),
each with its own version and tag. Releasing `sdk` also bumps the dependent (`swap`),
so it never stays pinned to a stale SDK:
