...
describe("arkade delegate (covenant batch refresh) — intent submission", () => {
    ...
    beforeAll(async () => {
        serverXOnlyPubkey = hex
            .decode((await arkProvider.getInfo()).signerPubkey)
            .slice(1);
...
function buildDelegateHandler(opts: {
    ...
    return {
        ...
        async onTreeSigningStarted(event, vtxoTree) {
            ...
            const xOnlyPubkeys = event.cosignersPublicKeys.map((k: string) =>
                k.slice(2)
            );
            ...
            if (!shared?.amount)
                throw new Error("missing shared output amount");
